...
$sql = mysqli_query($conexao,"SELECT * FROM usuario WHERE id = '".$_GET['id']."';");
...
<div class="page-header panel panel-default col-md-10 col-xs-12 text-center">
	...
	<h2 class="center cadTit">Editar Perfil - <?php echo $row['nome']." ".$row['sobrenome']; ?></h2>
		...
		<form method="POST" action="atualizaUser.php">
		...
		<div class="row"> 
			...
			<div class="form-group col-md-10 col-xs-12">
				...
				<input type="email" class="form-control" name="email" value="<?php echo $row["email"]; ?>" maxlength="64" required>
			</div>
